...
def demo_optimizaciones():
    """
    Demuestra diferentes configuraciones de optimización y compara su rendimiento.
    """
    configs = {
        "Base": {},
        "4-bit Quant": {"cuantizacion": True, "bits": 4},
        "Flash+Sliding": {"flash_attention": True, "sliding_window": 512},
        "Full Optim": {"cuantizacion": True, "bits": 4, "flash_attention": True, "sliding_window": 512}
    }
    
    texto_prueba = "La inteligencia artificial es" * 50  # Texto largo para prueba
    dispositivo = "cuda" if torch.cuda.is_available() else "cpu"
    
    resultados = {}
    
    for nombre, config in configs.items():
        print(f"\nEvaluando configuración: {nombre}")
        modelo, tokenizador = cargar_modelo_optimizado("facebook/opt-1.3b", config)
